...
from metric import *
from models.GATCNN51 import GATEmbedding
from torch.utils.data import WeightedRandomSampler
from config.data_config import config
from torch.utils.data import DataLoader
cf = config()

# MIE-GAT using only image features on the LIDC-IDRI dataset
def main(args_opt):
    ...
    for fold in range(1, args_opt.cross_num+1):
        ...
        train_loader = DataLoader(dataset_train, batch_size=cf.gat_batch_size, shuffle=shuffle_train, sampler=sampler_train,
                                  num_workers=args_opt.num_workers)
        valid_loader = DataLoader(dataset_valid, batch_size=cf.gat_batch_size, shuffle=shuffle_val, sampler=sampler_val,
                                num_workers=args_opt.num_workers)
        test_loader = DataLoader(dataset_test, batch_size=cf.gat_batch_size, shuffle=shuffle_test, sampler=sampler_test,
                                  num_workers=args_opt.num_workers)

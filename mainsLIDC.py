...
# MIE-GAT with attribute features on the LIDC-IDRI dataset
def _relabel_character(character):
    le = LabelEncoder()
    for i in range(character.shape[1] - 1):
        original_values = character[:, i].copy()
        character[:, i] = le.fit_transform(character[:, i])
        character[:, i] = character[:, i].astype(int)
        min_val, max_val = character[:, i].min(), character[:, i].max()
        print(f"Column {i}: Original min value = {original_values.min()}, max value = {original_values.max()}")
        print(f"Column {i}: Encoded min value = {min_val}, max value = {max_val}")
    return character
...
def main(args_opt):
    ...
    files = numpy.load(".data/32/cubes_12vs45.npy")
    character = numpy.load(".data/32/chara_12vs45.npy")
    character = numpy.delete(character, 0, axis=1)
    character = _relabel_character(character)
    files = files.reshape(files.shape[0], 32, 32, 32)
    y = numpy.load("./32/y_12vs45.npy")
    X_train_val, y_train_val, char_train_val = files, y, character
    kf = KFold(n_splits=5, shuffle=True, random_state=42)
    train_val_index = kf.split(X_train_val)

    for fold, (train_index, val_index) in enumerate(train_val_index):
        print(f"train: {len(train_index)}, val: {len(val_index)}, test: {len(val_index)}")
        ...
        X_train, y_train, char_train = X_train_val[train_index], y_train_val[train_index], char_train_val[train_index]
        X_val, y_val, char_val = X_train_val[val_index], y_train_val[val_index], char_train_val[val_index]

        dataset = GATData
        dataset_train = dataset(X_train, y_train, char_train, log=logger, partition='train', fold=fold, split=args_opt.split)
        dataset_valid = dataset(X_val, y_val, char_val, log=logger, partition='val', fold=fold, split=args_opt.split)
        dataset_test = dataset(X_val, y_val, char_val, log=logger, partition='test', fold=fold, split=args_opt.split)
        ...
        if args_opt.mode == 'eval' or args_opt.resume == True:
            ...
            logger.info('Please check mode and resume.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_ids', type=list, default=[0,1], help='number of gpu')
    ...
    parser.add_argument('--path_root', type=str, default='/media/data1/classification/Mid7/',
